    ...
    if (m_cwRunningParaPtr->baseTime == 0) {
        ...
        if (iMaintain < 0) {
            ...
            if (bCanOpen
                ...
                auto orderlist = EasyInputMultiOrder(m_cwStrategyParameter.Instrument.c_str(),
                    iVol, dbOrderPrice, m_cwStrategyParameter.OpenCloseMode, cwInsertOrderType::cwInsertLimitOrder);
        ...
        if (iMaintain > 0) {
            ...
            if (bCanOpen
                ...
                auto orderlist = EasyInputMultiOrder(m_cwStrategyParameter.Instrument.c_str(),
                    iVol * (-1), dbOrderPrice, m_cwStrategyParameter.OpenCloseMode, cwInsertOrderType::cwInsertLimitOrder);
                for (auto it = orderlist.begin();
                    it != orderlist.end(); it++) {
                    cwOrderPtr orderptr = *it;
                    if (orderptr.get() != NULL) {
